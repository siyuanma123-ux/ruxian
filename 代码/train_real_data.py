...
def parse_args():
    parser = argparse.ArgumentParser(description='使用真实数据集训练跨模态乳腺癌诊断模型')
    
    # 数据参数
    parser.add_argument('--mammo_metadata', type=str, 
                       default='x光数据集/manifest-1761561543948/metadata.csv',
                       help='X光数据metadata.csv路径')
    parser.add_argument('--mammo_root', type=str,
                       default='x光数据集/manifest-1761561543948',
                       help='X光数据根目录')
    parser.add_argument('--patho_root', type=str,
                       default='病理数据集/BreaKHis_v1',
                       help='病理数据根目录')
    parser.add_argument('--use_pathology', action='store_true', default=True,
                       help='是否使用病理数据')
...
def train_epoch(model, dataloader, optimizer, device, epoch):
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        # 更新进度条
        current_acc = correct_cls / total_samples if total_samples > 0 else 0.0
        pbar.set_postfix({
    ...
    return total_loss, loss_dict_avg, accuracy
...
def main():
    ...
    train_dataset = CBISBreakHisDataset(
        mammo_metadata=args.mammo_metadata,
    ...
    val_dataset = CBISBreakHisDataset(
        mammo_metadata=args.mammo_metadata,
        mammo_root=args.mammo_root,
        patho_root=args.patho_root,
        img_size=args.img_size,
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # 训练
        train_loss, train_loss_dict, train_acc = train_epoch(
